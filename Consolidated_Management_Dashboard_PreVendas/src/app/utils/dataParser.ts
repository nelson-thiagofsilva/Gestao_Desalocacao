...
export interface PreVendaRow {
  cliente: string;
  proposta: string; // = projeto (contém "PV -")
  custoTotal: number;
  totalHoras: number;
  projetoVendido: boolean;
}

export function consolidatePreVendas(rawData: RawDataRow[]): PreVendaRow[] {
  const pvRows = rawData.filter(r => r.projeto.includes('PV -'));

  const map = new Map<string, { custoTotal: number; totalHoras: number }>();

  pvRows.forEach(r => {
    const key = `${r.cliente}||${r.projeto}`;
    if (!map.has(key)) {
      map.set(key, { custoTotal: 0, totalHoras: 0 });
    }
    const entry = map.get(key)!;
    entry.custoTotal += r.custo_projeto;
    entry.totalHoras += r.qtdehoras;
  });

  const result: PreVendaRow[] = [];
  map.forEach((value, key) => {
    const [cliente, proposta] = key.split('||');
    result.push({
      cliente,
      proposta,
      custoTotal: value.custoTotal,
      totalHoras: value.totalHoras,
      projetoVendido: false,
    });
  });

  return result.sort((a, b) => a.cliente.localeCompare(b.cliente) || a.proposta.localeCompare(b.proposta));
}
